...
/* ============================================================
   app.js — Application logic for MPI 4
   ...
   The nine stages run the Discovery Learning syntax and the TBLT cycle
   at the same time: Stages 1-3 are the pre-task, Stages 4-6 are the
   language focus, and Stage 7 is the task itself — rehearse, plan,
   report.
...
var STORAGE_KEY = 'eng-present-continuous-v1';
...
/* ============================================================
   5. STAGE: ORIENTATION  (TBLT: pre-task)
   ============================================================ */
...
function renderOrientation(container) {
  ...
  /* The target task is shown here, before any language work. That is what
     makes the six stages in between read as preparation rather than as
     exercises — and it is the one thing TBLT will not let you leave to
     the end. */
  var taskBulletsHTML = D.taskBullets
    .map(function (b) {
      return '<li>' + b + '</li>';
    })
    .join('');
...
/* Every clickable word, target and decoy alike. */
function noticeTokens() {
  var out = [];
  DATA.noticeIt.text.forEach(function (line, li) {
    line.forEach(function (tok, ti) {
      if (typeof tok === 'object' && tok.w) out.push({ key: noticeKey(li, ti), tok: tok });
    });
  });
  return out;
}
...
/*
 * The decoys — plain simple-present verbs and repeat signals — are
 * clickable but are NOT what the learner is being asked to find. They are
 * the whole reason this stage does any work: a learner who marks them has
 * spotted "a word doing a job" rather than "a word showing a job in
 * progress", and the check is where that difference first bites.
 */
function noticeScore() {
  var targets = noticeTokens().filter(function (t) {
    return !t.tok.decoy;
  });
  var hits = targets.filter(function (t) {
    return State.noticeMarked[t.key];
  }).length;
  return { hits: hits, total: targets.length };
}

function noticeDecoysMarked() {
  return noticeTokens().filter(function (t) {
    return t.tok.decoy && State.noticeMarked[t.key];
  }).length;
}
...
function renderNoticeIt(container) {
    ...
    .map(function (line, li) {
      var inner = line
        .map(function (tok, ti) {
          ...
          if (checked) {
            /* Two different errors must not look the same. Red means
               "you walked past this"; a marked decoy is the opposite
               mistake and gets its own amber, from mpi-4/styles.css. A
               decoy left alone is simply an ordinary word again, so it
               gets no colour at all. */
            if (tok.decoy) cls += marked ? ' is-stray' : '';
            else cls += marked ? ' is-hit' : ' is-miss';
          } else if (marked) {
            cls += ' is-marked';
          }
          return (
    ...
    feedbackHTML =
      buildFeedbackBox(
        ...
        '<strong>You marked ' +
          ...
          ' of the ' +
          score.total +
          ' words that show something in progress.</strong> ' +
          'The ones you walked past are outlined in <strong>red</strong> above' +
          (strays
            ? ', and the ' +
              strays +
              ' word' +
  ...
  container.innerHTML = stageShell(
    ...
    '<div class="panel">' +
      ...
      '</p>' +
      '<div class="live-card">' +
      '<span class="live-card__label">' +
      '<span class="live-card__dot" aria-hidden="true"></span>' +
      D.liveLabel +
      '</span>' +
      '<span class="live-card__meta">' +
      D.liveMeta +
      '</span>' +
...
/* ============================================================
   11. STAGE: GO LIVE  (generalisation · the TBLT task)

   Three blocks in one stage, and the order is the point:
     A. Rehearse — controlled practice on somebody else's sentences
     B. Plan     — the learner drafts their own broadcast
     C. Report   — self-check, then read it out loud

   Blocks B and C stay hidden until every rehearsal sentence is right.
   A script planned before the rehearsal is a script written with the
   old habits still in place.
   ============================================================ */
...
function renderGeneralization(container) {
  ...
  if (allBuilt) {
    ...
    taskHTML =
      ...
      rubricHTML +
      '</div>' +
      '<div class="on-air-note">' +
      '<span class="on-air-note__label">' +
      '<span class="live-card__dot" aria-hidden="true"></span>Read it out loud</span>' +
      '<p style="margin:0;">' +
      esc(D.onAirNote) +
      '</p></div></div>' +
      '<div class="btn-group btn-group--end">' +
...
function collectScores() {
  ...
  return [
    ...
    { label: 'Go Live — rehearsal', correct: builder.correct, total: builder.total },
  ];
}
...
function renderResults(container) {
  ...
  /* The broadcast script is never scored automatically — it comes back
     here so the learner, and their teacher, can read it against the same
     rubric the learner used on themselves. */
  var writingHTML = State.ownWriting.trim()
    ? '<div class="panel">' +
      '<h3>Your broadcast script</h3>' +
      '<p class="own-writing">' +
...
var RULE_NAMES = {
  meaning: 'now, or every day',
  subject: 'which helper',
  spelling: 'spelling the -ing',
  negative: 'saying no',
  question: 'asking',
